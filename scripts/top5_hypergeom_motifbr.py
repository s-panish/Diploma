...
# ================= НАСТРОЙКИ =================
INPUT_FILE = 'motifbreakr_with_cell_lines_unique.tsv'
OUTPUT_FILE = 'table2_tf_top5_frequency_enrichment_motifbr.tsv'
TOP5_PAIRS_FILE = 'top5_tf_cell_lines.tsv'
# =============================================
...
# Убираем пары, где нет данных
tf_cell_agg = tf_cell_agg[tf_cell_agg['K'] > 0].copy()
tf_cell_agg['ratio'] = tf_cell_agg['k'] / tf_cell_agg['K']

print(f"   Уникальных пар TF×ткань с данными: {len(tf_cell_agg)}")
...
n = tf_cell_agg['in_top5'].sum()  # n: всего пар в топ-5%
N = len(tf_cell_agg)              # N: всего пар в датасете
...
# Сортируем по убыванию ratio для удобства чтения
...
tf_agg = tf_cell_agg.groupby('TF')['in_top5'].agg(
    ...
    K=('size')          # K: число тканей, где ТФ НЕ в топ-5%
).reset_index()
