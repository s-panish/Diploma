...
INPUT_FILE = Path('motifbreakr_with_cell_lines_unique.tsv')
...
OUTPUT_FILE = Path('motifbreakr_with_cell_lines_unique_broken_expression_ge1.tsv')
UNMATCHED_FILE = Path('motifbreakr_with_cell_lines_unique_broken_unmatched_expression.tsv')
LOW_EXPRESSION_FILE = Path('motifbreakr_with_cell_lines_unique_broken_expression_lt1.tsv')
DIAGNOSTICS_FILE = Path('motifbreakr_with_cell_lines_unique_broken_expression_diagnostics.tsv')

# ======================================================


# Ручные соответствия контекстов: как в твоей таблице -> как в HPA.
...
# Ручные соответствия TF: как в motifbreakR/HOCOMOCO -> как Gene name в HPA.
DEFAULT_TF_ALIASES = {
    'AP2D': 'TFAP2D',
    'BMAL1': 'BMAL1',
    'ZF64A': 'ZFP64',
...
def context_keys(context, source):
    ...
    manual_general = {
        ...
        'cd4 t cells': ['cd4 t cell', 'naive cd4 t-cell', 'memory cd4 t-cell'],
        'cd8 t cells': ['cd8 t cell', 'naive cd8 t-cell', 'memory cd8 t-cell'],
...
def make_broken_mask(df):
    ...
    for col in COMPARISON_COLUMNS:
        ...
        mask = mask | values.isin(BROKEN_VALUES)
...
def load_tf_map():
    ...
    if TF_LIST_FILE.exists():
        ...
        tf_df = tf_df[(tf_df['curated:uniprot_id'] != '') & (tf_df['auto:gene_symbol'] != '')].copy()
...
def build_attempts(raw_context, aliases):
    ...
    if '__' in raw_context:
        ...
        for source in ['cell_line_cancer', 'tissue_consensus', 'brain_pfc_subregion', 'brain_subregion', 'brain_region']:
...
def main():
    ...
    print('\n2. Использование всех сайтов без фильтрации по motifbreakR')
    broken_df = df.copy()
    print(f'   Строк взято в анализ: {len(broken_df)}')

    print('\n3. Загрузка TF-словаря')
    tf_map = load_tf_map()

    broken_df['_hpa_gene_symbol'] = broken_df[gene_col].apply(lambda x: map_tf(x, tf_map))
    broken_df['_gene_key'] = broken_df['_hpa_gene_symbol'].apply(normalize_gene)
    ...
    print('\n6. Поиск экспрессии для строк со сломанными сайтами')
    ...
    broken_df['expression_value'] = expression_values
    broken_df['expression_unit'] = expression_units
    broken_df['expression_source'] = expression_sources
    broken_df['matched_hpa_context'] = matched_contexts
    broken_df['_expression_value_numeric'] = expression_values_numeric
    broken_df['_checked_hpa_gene_symbol'] = checked_genes
    broken_df['_diagnostic_status'] = statuses

    matched_mask = broken_df['_expression_value_numeric'].notna()
    expressed_mask = matched_mask & (broken_df['_expression_value_numeric'] >= EXPRESSION_THRESHOLD)
    low_expression_mask = matched_mask & (broken_df['_expression_value_numeric'] < EXPRESSION_THRESHOLD)
    ...
    output_df = broken_df.loc[expressed_mask].drop(columns=helper_cols, errors='ignore').copy()
    output_df.to_csv(OUTPUT_FILE, sep='\t', index=False, na_rep='')
    print(f'   Итоговая таблица: {OUTPUT_FILE}')

    low_expression_df = broken_df.loc[low_expression_mask].drop(columns=helper_cols, errors='ignore').copy()
    ...
    unmatched_df = broken_df.loc[unmatched_mask, [
        gene_col,
        context_col,
        '_checked_hpa_gene_symbol',
        '_diagnostic_status'
