...
df = df.dropna(subset=["K/N", "TF_family", "TF_type"]).copy()
...
for i, family in enumerate(families, start=1):
    ...
    # считаем число ТФ и ПТФ в данном семействе
    ...
    # оформление
    ax.set_title(family, fontsize=13, pad=12, color="black")
    ax.set_xlabel("")
    ...
    if i % 20 == 0 or i == len(families):
        print(f"Обработано {i} из {len(families)}")
