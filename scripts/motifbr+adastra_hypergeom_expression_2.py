...
OUTPUT_FILE = 'table_tf_broken_site_enrichment_adastra_expression_ge1_2.tsv'
BROKEN_SITES_FILE = 'broken_sites_adastra_expression_ge1_2.tsv'
TF_CELL_LINE_COUNTS_FILE = 'tf_cell_line_broken_counts_adastra_expression_ge1_2.tsv'
...
# Фильтр по числу сломанных и несломанных сайтов у TF
MIN_BROKEN_SITES = 1
MIN_INTACT_SITES = 1
...
print(f'   TF до фильтрации по k и K: {len(tf_agg)}')

# Фильтрация по k и K:
# k > 1 — больше одного сломанного сайта
# K > 1 — больше одного несломанного сайта
tf_agg = tf_agg[
    (tf_agg['k'] > MIN_BROKEN_SITES) &
    (tf_agg['K'] > MIN_INTACT_SITES)
].copy()

if len(tf_agg) == 0:
    raise ValueError(
        f'После фильтрации k > {MIN_BROKEN_SITES} и K > {MIN_INTACT_SITES} '
        'не осталось TF для гипергеометрического теста.'
    )

print(f'   TF после фильтрации по k > {MIN_BROKEN_SITES} и K > {MIN_INTACT_SITES}: {len(tf_agg)}')
